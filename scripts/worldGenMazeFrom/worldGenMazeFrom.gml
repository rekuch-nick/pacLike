function worldGenMazeFrom(cx, cy){
	...
	while(last >= 0){
		...
		var tttx = cx; var ttty = cy;
		
		if(r == 1){ tty = cy - 1; ty = cy - 3; ttty = cy - 2; }
		if(r == 2){ ttx = cx + 1; tx = cx + 3; tttx = cx + 2; }
		if(r == 3){ tty = cy + 1; ty = cy + 3; ttty = cy + 2; }
		if(r == 4){ ttx = cx - 1; tx = cx - 3; tttx = cx - 2; }
		d[r] = true;
